...
public class MainActivity extends AppCompatActivity {
    ...
    private void page(String title) {
        ...
        bar.addView(tv,new LinearLayout.LayoutParams(0,55,1));
        root.addView(bar);
    }
    ...
    private void addLogo() {
        ...
        int id = getResources().getIdentifier(
                "edison_solar_logo","drawable",getPackageName());
        if (id != 0) im.setImageResource(id);
        im.setAdjustViewBounds(true);
        im.setScaleType(ImageView.ScaleType.CENTER_INSIDE);

        root.addView(im,new LinearLayout.LayoutParams(-1,135));
    }
    ...
    private void home() {
        ...
        root.addView(button("📱 WhatsApp Report",GREEN,
    ...
    private void companies() {
        ...
        root.addView(button("+ Add Company",BLUE,
        ...
        for (Company c : db.companies()) {
            root.addView(card(
                    c.name +
                    "\nPhone: " + c.phone +
                    "\nProjects: " + db.countProjects(c.id) +
                    " | kW: " + DBHelper.fmt(db.companyKw(c.id)) +
                    "\nCollection: ₹" +
                    DBHelper.fmt(db.companyCollection(c.id)) +
                    "\nExpenses: ₹" +
                    DBHelper.fmt(db.companyExpenses(c.id)) +
                    "\nPending: ₹" +
                    DBHelper.fmt(db.companyValue(c.id)-db.companyCollection(c.id)) +
                    "\nProfit: ₹" +
                    DBHelper.fmt(db.companyValue(c.id)-db.companyExpenses(c.id))
            ));

            LinearLayout row = new LinearLayout(this);

            row.addView(button("Edit",BLUE,
                    v -> companyDialog(c)),
                    new LinearLayout.LayoutParams(0,52,1));

            row.addView(button("Delete",RED,
                    v -> confirm("Delete company?",() -> {
    ...
    private void companyDialog(Company old) {
        ...
        EditText name = field("Company Name",
                old == null ? "" : old.name);
        ...
        AlertDialog d = new AlertDialog.Builder(this)
                .setTitle(old == null ? "Add Company" : "Edit Company")
                .setView(l)
        ...
        d.setOnShowListener(x ->
                d.getButton(-1).setOnClickListener(v -> {
                    ...
                    if (old == null)
                        db.addCompany(n,phone.getText().toString());
                    else
                        db.updateCompany(
                                old.id,n,phone.getText().toString());
    ...
    private void projects() {
        ...
        root.addView(button("+ Add Project",BLUE,
        ...
        for (Project p : db.projects()) {
            ...
            root.addView(button("Open Project",BLUE,
                    v -> projectMenu(p)));
        }
    }
    ...
    private void projectDialog(Project old) {
        ...
        if (cs.isEmpty()) {
            Toast.makeText(this,
                    "Add company first",Toast.LENGTH_LONG).show();
            return;
        }
        ...
        for (Company c : cs) names.add(c.name);
        ...
        EditText phone = field("Customer Phone",
                old == null ? "" : old.phone);
        EditText site = field("Site Address",
                old == null ? "" : old.site);
        EditText kw = field("Solar kW",
                old == null ? "" : String.valueOf(old.kw));
        EditText amount = field("Project Amount",
                old == null ? "" : String.valueOf(old.amount));
        EditText date = field("Installation Date",
                old == null ? today() : old.date);
        EditText work = field("Site Work Detail",
                old == null ? "" : old.work);
        ...
        if (old != null && "Work Going On".equals(old.status))
        ...
        dateBtn.setText("📅 Choose Installation Date");
        dateBtn.setOnClickListener(v -> chooseDate(date));
        l.addView(dateBtn);

        AlertDialog d = new AlertDialog.Builder(this)
                .setTitle(old == null ? "New Project" : "Edit Project")
                .setView(l)
                .setNegativeButton("Cancel",null)
                .setPositiveButton("Save",null)
                .create();
        ...
        d.setOnShowListener(x ->
                d.getButton(-1).setOnClickListener(v -> {
                    ...
                    Company c = cs.get(sp.getSelectedItemPosition());
                    ...
                    String status =
                            going.isChecked() ? "Work Going On" : "Pending";
                    ...
                    if (old == null) {
                        db.addProject(
                                ...
                                number(kw),number(amount),
                                date.getText().toString(),
                                status,
                                work.getText().toString());
                    } else {
                        db.updateProject(
                                old,c.id,c.name,cust,
                                phone.getText().toString(),
                                site.getText().toString(),
                                number(kw),number(amount),
                                date.getText().toString(),
    ...
    private void projectMenu(Project p) {
        new AlertDialog.Builder(this)
                ...
                .setItems(
                        ...
                        (d,w) -> {
                            if (w==0) projectDialog(p);
                            else if (w==1) projectPayments(p);
                            else if (w==2) projectExpenses(p);
                            else if (w==3) photos(p);
                            else if (w==4) gps(p);
                            else if (w==5) pdf(p);
                            else if (w==6) share(projectReport(p));
                            else confirm(
    ...
    private void projectPayments(Project p) {
        ...
        root.addView(button("+ Add Payment",GREEN,
                v -> paymentDialog(p)));
    }
    ...
    private void paymentDialog(Project p) {
        ...
        EditText amount = field("Payment Amount","");
        EditText note = field("Payment Note","");
    ...
    private void payments() {
        ...
        for (Project p : db.projects()) {
            root.addView(card(
                    p.number+" • "+p.customer+
                    "\nCollected: ₹"+DBHelper.fmt(db.collection(p.id))+
                    "\nPending: ₹"+DBHelper.fmt(db.pending(p.id))
            ));
            root.addView(button("Add Payment",GREEN,
                    v -> paymentDialog(p)));
    ...
    private void projectExpenses(Project p) {
        ...
        root.addView(button("+ Add Expense",ORANGE,
                v -> expenseDialog(p)));
    }
    ...
    private void expenseDialog(Project p) {
        ...
        EditText amount = field("Expense Amount","");
        EditText type = field("Expense Type","Other");
        EditText note = field("Expense Note","");
    ...
    private void calendar() {
        ...
        root.addView(button("📅 Select Date",PURPLE,
                v -> {
    ...
    private void calendarDay(String date) {
        ...
        if (!found)
            root.addView(card("No project work on this date."));
    }
    ...
    private void photos(Project p) {
        ...
        root.addView(card(
                p.customer+"\n"+p.site+
                "\nSaved Photos: "+db.photos(p.id).size()
        ));
    ...
    private void takePhoto() {
        ...
        try {
            ...
            File f = new File(
                    dir,
                    "EDISON_"+System.currentTimeMillis()+".jpg");
    ...
    protected void onActivityResult(
        ...
        if (requestCode == 101) {
            ...
            if (photoProject != null && cameraUri != null) {
                db.addPhoto(
                        photoProject.id,
                        cameraUri.toString());
            }
